...
-- Run this script to implement database-level business logic
...
BEGIN
    -- Note: This assumes orders or order_items has branch_id
...
BEGIN
    ...
    IF v_order_status IN ('shipped', 'delivered', 'cancelled') THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = CONCAT('Cannot modify items in order with status: ', v_order_status);
    END IF;

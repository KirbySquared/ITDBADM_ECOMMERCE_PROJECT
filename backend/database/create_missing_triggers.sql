...
-- Run this script to implement database-level business rules
...
BEGIN
    ...
    IF NEW.status = 'cancelled' AND OLD.status != 'cancelled' THEN
        ...
        -- Get branch_id from order (assuming orders table has branch_id)
        -- If orders table doesn't have branch_id, we need to get it from order_items or another source
        -- For now, we'll restore stock to the branch where the order was placed
        -- This assumes order_items or orders has branch_id - adjust based on your schema
...
BEGIN
    ...
    IF OLD.status != NEW.status THEN
        INSERT INTO transaction_log (
            ...
            @audit_user_id, -- Set this variable before updating order
            NOW()
        );
    END IF;
END$$
...
-- Note: This assumes a currencies table exists. Adjust table/column names as needed.
...
-- VERIFY TRIGGERS
-- ========================================
SHOW TRIGGERS WHERE `Table` IN ('order_items', 'orders', 'payments', 'currencies', 'products');

SELECT 'All missing triggers created successfully!' AS Status;
